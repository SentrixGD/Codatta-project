...
class SwinModel(nn.Module):
    def __init__(
        ...
        self.patch_embed = Patching(self.input_channels, dim)

        self.stages = nn.ModuleList()
        self.merges = nn.ModuleList()

        self.dims = [dim * 2**i for i in range(stage_num)]
    ...
    def _run_stage(self, x, blocks, stage_idx):
    ...
    def forward(self, x, use_checkpoint=False, active_heads=None):
        ...
        for stage_idx, blocks in enumerate(self.stages):
            if use_checkpoint and self.training:
                x = checkpoint(
                    self._run_stage, x, blocks, stage_idx,
                    use_reentrant=False,
                )
            else:
                x = self._run_stage(x, blocks, stage_idx)

        x = self.gap(x)
        x = self.flatten(x)
        x = self.shared_mlp(x)

        compute_all = active_heads is None
        logits = {}

        food_type_logits = self.food_type_head(x)
        logits["food_type_logits"] = food_type_logits

        if compute_all or "ingredients" in active_heads:
            logits["ingredients_logits"] = self.ingredients_head(x)
        if compute_all or "portions" in active_heads:
            portion_embedding = self.portion_shared_head(x)
            logits["portion_weight_logits"] = self.portion_weight_output(portion_embedding)
            logits["portion_presence_logits"] = self.portion_presence_output(portion_embedding)
        if compute_all or "dish_name" in active_heads:
            logits["dish_name_logits"] = self.dish_names_head(x)
        if compute_all or "cooking_method" in active_heads:
            logits["cooking_method_logits"] = self.cooking_method_head(x)
        if compute_all or "calories" in active_heads:
            logits["calories_logits"] = self.calories_head(x)
        if compute_all or "fats" in active_heads:
            logits["fats_logits"] = self.fats_head(x)
        if compute_all or "carbohydrates" in active_heads:
            logits["carbohydrates_logits"] = self.carbohydrates_head(x)
        if compute_all or "proteins" in active_heads:
            logits["proteins_logits"] = self.proteins_head(x)
        if compute_all or "binary" in active_heads:
            logits["binary_logits"] = self.binary_head(x)
